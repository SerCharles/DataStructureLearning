...
int main()
{
    LNodeList* a_list;
    LNodeList* b_list;
    a_list = new LNodeList();
    b_list = new LNodeList();
    ...
    for(int i = 1; i <= a_length; i++)
    {
        ...
        LNode* new_node;
        cin >> new_number;
        new_node = new LNode(new_number);
        a_list -> m_Append(a_list -> m_Tail, new_node);
    }
    cin >> b_length;
    for(int i = 1; i <= b_length; i++)
    {
        ...
        cin >> new_number;
        new_node = new LNode(new_number);
        b_list -> m_Append(b_list -> m_Tail, new_node);
    }
    LNodeList* answer_list = f_GetIntersection(*a_list, *b_list);
    if(answer_list == NULL)
    {
        cout<<"0";
    }
    ...
    {
        cout << answer_list ->m_GetLength() << endl;
        answer_list ->m_OutPut();
    }
    return 0;
