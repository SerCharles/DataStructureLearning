...
public:
    ...
    void m_DeleteNode(BBinaryNode<Data>* ready_to_delete)
    {
        ...
        {
            BBinaryNode<Data>* new_current = m_FindLeftEnd(ready_to_delete -> m_RightSon);
...
int main()
{
    ...
    if(the_tree == NULL)
    {
        the_tree = new BBinarySortTree<int>();
        the_tree -> m_Root = new_node;
    }
    else
    {

    BBinaryNode<int>* find_place = the_tree -> m_FindNode(new_node);
    int a = new_node -> m_Compare(find_place);
        if(a == -1)
        {
            the_tree -> m_AddNode(new_node, find_place, 0);
        }
        else if(a == 1)
        {
            the_tree -> m_AddNode(new_node, find_place, 1);
        }
    }
