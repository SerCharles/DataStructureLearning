...
void DivideTree(BBinaryNode<int>* current_node, BBinarySortTree<int>*& tree_smaller, BBinarySortTree<int>*& tree_bigger, BBinaryNode<int>* tag_node)
{
    ...
    {
        int num = current_node -> m_GetData();
        BBinaryNode<int>* new_node = new BBinaryNode<int>(num);
        if(current_node -> m_Compare(tag_node) <= 0)
        {
            if(tree_smaller == NULL)
            {
                tree_smaller = new BBinarySortTree<int>();
                tree_smaller -> m_Root = new_node;
            }
            else
            {
                tree_smaller -> m_InsertNewNode(new_node);
            }
        }
        else
        {
            if(tree_bigger == NULL)
            {
                tree_bigger = new BBinarySortTree<int>();
                tree_bigger -> m_Root = new_node;
            }

            else
            {
                tree_bigger -> m_InsertNewNode(new_node);
            }
        }
        DivideTree(current_node -> m_LeftSon, tree_smaller, tree_bigger, tag_node);
        DivideTree(current_node -> m_RightSon, tree_smaller, tree_bigger, tag_node);
    }
}
...
int main()
{
    ...
    BBinarySortTree<int>* tree_smaller = NULL;
    BBinarySortTree<int>* tree_bigger = NULL;
    int m;
    cin >> m;
    BBinaryNode<int>* tag_node = new BBinaryNode<int>(m);
    DivideTree(the_tree -> m_Root, tree_smaller, tree_bigger, tag_node);
    
    if(tree_smaller != NULL)
    {
        tree_smaller -> m_OutPut(tree_smaller -> m_Root);
    }
    ...
    if(tree_bigger != NULL)
    {
        tree_bigger -> m_OutPut(tree_bigger -> m_Root);
    }
    return 0;
